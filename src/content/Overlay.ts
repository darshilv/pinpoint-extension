...
export class Overlay {
  ...
  #handleMouseOver(e: MouseEvent): void {
    ...
    const target = path.find(
      (el): el is Element => el instanceof Element && !isAnnotatorElement(el)
    );
  ...
  #dispatchElementClick(target: Element): void {
    ...
    document.dispatchEvent(
      new CustomEvent<ElementClickDetail>(EVENTS.ELEMENT_CLICK, {
        bubbles: true,
        composed: true,
        detail,
      })
  ...
  #renderMarkers(): void {
    ...
    const markerMarkup = this.#activeMarkers
      ...
      .map((marker) => {
        const top = Math.max(
          10,
          Math.min(window.innerHeight - 42, marker.rect.y + marker.rect.height - 18)
        );
        const left = Math.max(
          10,
          Math.min(window.innerWidth - 42, marker.rect.x + marker.rect.width - 18)
        );
