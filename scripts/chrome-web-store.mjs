...
function summarizePermissions(manifest) {
  return {
    permissions: (manifest.permissions ?? []).map((permission) => ({
      key: permission,
      justification: permissionJustifications[permission] ?? 'Add a reviewer-facing justification before submission.',
    })),
    hostPermissions: [...(manifest.host_permissions ?? []), ...(manifest.optional_host_permissions ?? [])].map((permission) => ({
      key: permission,
      justification: hostPermissionJustifications[permission] ?? 'Explain exactly which sites need this access and why.',
    })),
  }
}
...
function getWarnings(manifest) {
  ...
  if ((manifest.host_permissions ?? []).includes('<all_urls>')) {
    warnings.push('`<all_urls>` will trigger extra review scrutiny. Replace it with optional per-site access before submission.')
  }
  if ((manifest.optional_host_permissions ?? []).length === 0) warnings.push('No optional host permissions declared. Per-site persistence requests will not work.')
  if (!fileExists(path.join(docsDir, 'privacy-policy.md'))) {
    warnings.push('Missing `docs/chrome-web-store/privacy-policy.md`.')
  }
  if (!fs.existsSync(screenshotsDir) || fs.readdirSync(screenshotsDir).filter((name) => name.endsWith('.png')).length === 0) {
...
function validateManifest(manifest, preferDist = false) {
  ...
  if (!manifest.background?.service_worker) issues.push('Manifest is missing a background service worker.')
...
${permissionSummary.hostPermissions.length === 0
    ? '- None'
    : permissionSummary.hostPermissions.map(({ key, justification }) => `- \`${key}\`: ${justification}`).join('\n')}
...
${permissionSummary.permissions.concat(permissionSummary.hostPermissions).map(({ key, justification }) => `- \`${key}\`: ${justification}`).join('\n')}
